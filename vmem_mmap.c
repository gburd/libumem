...
vmem_mmap_top_alloc(vmem_t *src, size_t size, int vmflags)
{
	...
	if (buf != MAP_FAILED) {
		 ...
		 *
		 * Do NOT restore errno here.  mmap() has just said WHY it
		 * failed and that is the most useful fact available: ENOMEM
		 * from vm.max_map_count exhaustion is indistinguishable from
		 * ordinary out-of-memory unless the caller can see it.  This
		 * used to overwrite it with the value on entry, so a caller got
		 * NULL with a stale errno -- measured as "FIRST FAILURE at
		 * 8269MB (errno=0 Success)", a failed allocation reporting
		 * success.  That is why hitting the address-space ceiling
		 * presented as "libumem is slower" rather than "libumem could
		 * not get memory".
		 ...
		 * vmem_populate() now reports it and returns ENOMEM.
		 *
		 * Success paths still restore errno: a successful allocation
		 * must not perturb it.
		 */
		return (NULL);
	}
