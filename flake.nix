        ...
        devShells = rec {
          ...
          native = pkgs.mkShell {
            ...
            shellHook = ''
              ...
              echo "  nix run .#test     Main suite (make check)"
              echo "  nix run .#unit     Unit tests (62+ tests)"
              echo "  nix run .#prop     Property-based tests"
              echo "  nix run .#integ    Integration tests"
              echo "  nix run .#perf     Benchmarks"
              echo ""
              echo "Manual testing:"
              echo "  make check         Main test suite"
              echo "  ./test/test_main   Unit tests directly"
              echo ""
              echo "Quality tools:"
              echo "  ./scripts/generate-coverage-report.sh    Coverage"
              echo "  ./scripts/run-sanitizers.sh              ASan/TSan"
              echo ""
