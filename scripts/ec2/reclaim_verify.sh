#!/usr/bin/env bash
# P1.4/P1.5 verification: build, run the four reclaim regressions, then the
...
run() { echo "--- $1 ---"; shift; "$@"; echo "rc=$?"; }

run "P1.4 destroy" ./test/unit/.libs/repro_reclaim_destroy
run "P1.5a hash_guards" env UMEM_DEBUG=guards UMEM_OPTIONS="$RECL" ./test/unit/.libs/repro_reclaim_reuse hash_guards
run "P1.5b big_quantum" env UMEM_OPTIONS="$RECL" ./test/unit/.libs/repro_reclaim_reuse big_quantum
